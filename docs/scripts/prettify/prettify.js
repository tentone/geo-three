...
[], b=[], o=f[0]==="^", c=o?1:0, i=f.length;c<i;++c) 
			{
				var j=f[c];if (/\\[bdsw]/i.test(j)) {a.push(j);}
				else {var j=m(j), d;c+2<i&&"-"===f[c+1]?(d=m(f[c+2]), c+=2):d=j;b.push([j, d]);d<65||j>122||(d<65||j>90||b.push([Math.max(65, j)|32, Math.min(d, 90)|32]), d<97||j>122||b.push([Math.max(97, j)&-33, Math.min(d, 122)&-33]));}
			}b.sort(function(a, f) {return a[0]-f[0]||f[1]-a[1];});f=[];j=[NaN, NaN];for (c=0;c<b.length;++c) {i=b[c], i[0]<=j[1]+1?j[1]=Math.max(j[1], i[1]):f.push(j=i);}b=["["];o&&b.push("^");b.push.apply(b, a);for (c=0;c<
f.length;++c) {i=f[c], b.push(e(i[0])), i[1]>i[0]&&(i[1]+1>i[0]&&b.push("-"), b.push(e(i[1])));}b.push("]");return b.join("");
		} function y(a)
		{
			for (var f=a.source.match(/\[(?:[^\\\]]|\\[\S\s])*]|\\u[\dA-Fa-f]{4}|\\x[\dA-Fa-f]{2}|\\\d+|\\[^\dux]|\(\?[!:=]|[()^]|[^()[\\^]+/g), b=f.length, d=[], c=0, i=0;c<b;++c) {var j=f[c];j==="("?++i:"\\"===j.charAt(0)&&(j=Number(j.substring(1)))&&j<=i&&(d[j]=-1);} for (c=1;c<d.length;++c) {-1===d[c]&&(d[c]=++t);} for (i=c=0;c<b;++c)
			{
...
(j=Number(j.substring(1)))&&j<=i&&(f[c]="\\"+d[i]);
		...
		{
			var g=a[p];if (g.ignoreCase) {l=!0;}
			else if (/[a-z]/i.test(g.source.replace(/\\u[\da-f]{4}|\\x[\da-f]{2}|\\[^UXux]/gi, ""))) {s=!0;l=!1;break;}
		} for (var r=
...
"string") {c=!1;}
				...
				{
					var i=h[f.charAt(0)];if (i) {o=f.match(i[1]), b=i[0];}
					else {for (c=0;c<t;++c) {if (i=m[c], o=f.match(i[1])) {b=i[0];break;}}o||(b="pln");} if ((c=b.length>=5&&"lang-"===b.substring(0, 5))&&!(o&&typeof o[1]==="string")) {c=!1, b="src";}c||(r[f]=b);
				}i=d;d+=f.length;if (c) {c=o[1];var j=f.indexOf(c), k=j+c.length;o[2]&&(k=f.length-o[2].length, j=k-c.length);b=b.substring(5);B(l+i, f.substring(0, j), e, p);B(l+i+j, c, C(b, c), p);B(l+i+k, f.substring(k), e, p);}
				else {p.push(l+i, b);}
			}a.e=p;
		} var h={}, y;(function()
		{
			for (var e=a.concat(m),
				l=[], p={}, d=0, g=e.length;d<g;++d) {var r=e[d], n=r[3];if (n) {for (var k=n.length;--k>=0;) {h[n.charAt(k)]=r;}}r=r[1];n=String(r);p.hasOwnProperty(n)||(l.push(r), p[n]=q);}l.push(/[\S\s]/);y=L(l);
...
!k) {b=n;for (var o=void 0, c=b.firstChild;c;c=c.nextSibling) {var i=c.nodeType, o=i===1?o?b:c:i===3?N.test(c.nodeValue)?b:o:o;}b=(f=o===b?void 0:o)&&"CODE"===f.tagName;}b&&(k=f.className.match(g));k&&(k=k[1]);b=!1;for (o=n.parentNode;o;o=o.parentNode) {if ((o.tagName==="pre"||o.tagName==="code"||o.tagName==="xmp")&&o.className&&o.className.indexOf("prettyprint")>=0) {b=!0;break;}}b||((b=(b=n.className.match(/\blinenums\b(?::(\d+))?/))?b[1]&&b[1].length?Number(b[1]):!0:!1)&&D(n, b), d={g: k, h: n, i: b}, E(d));
				}
			}p<h.length?setTimeout(m,
				250):a&&a();
		} for (var e=[document.getElementsByTagName("pre"), document.getElementsByTagName("code"), document.getElementsByTagName("xmp")], h=[], k=0;k<e.length;++k) {for (var t=0, s=e[k].length;t<s;++t) {h.push(e[k][t]);}} var e=q, l=Date;l.now||(l={now: function() {return Number(new Date());}});var p=0, d, g=/\blang(?:uage)?-([\w.]+)(?!\S)/;m();
	};window.PR={
		createSimpleLexer: x,
